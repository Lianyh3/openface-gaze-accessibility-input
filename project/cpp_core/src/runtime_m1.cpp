...
void AddGridRegions(std::vector<TargetRegion>* out,
...
std::optional<TargetEvent> ParseTargetToEvent(std::int64_t timestamp_ms,
...
LinearNormalizer::LinearNormalizer(double x_min, double x_max, double y_min, double y_max, bool clamp)
...
std::pair<double, double> LinearNormalizer::Normalize(double x, double y) const {
...
KeyboardHitTester::KeyboardHitTester(std::vector<TargetRegion> regions) : regions_(std::move(regions)) {
  if (regions_.empty()) {
    throw std::invalid_argument("regions must not be empty");
  }
}

std::string KeyboardHitTester::HitTest(double x, double y) const {
...
KeyboardHitTester KeyboardHitTester::BuildDefault(const LayoutPreset& preset) {
  ...
  AddGridRegions(&regions, candidate_ids, 0.0, 1.0, 0.0, 0.24, 4);

  std::vector<std::string> key_ids = {
      "key:我", "key:今天", "key:想",   "key:去",   "key:你",   "key:现在",
      "key:需要", "key:请",  "key:帮", "key:打开", "key:发送", "key:消息",
  };
  AddGridRegions(&regions, key_ids, 0.0, 1.0, 0.24, 0.82, 4);
  ...
  AddGridRegions(&regions, action_ids, 0.0, 1.0, 0.82, 1.0, 4);

  return KeyboardHitTester(std::move(regions));
}

DwellDetectorCore::DwellDetectorCore(std::int64_t dwell_ms) : dwell_ms_(dwell_ms) {
  if (dwell_ms_ <= 0) {
    throw std::invalid_argument("dwell_ms must be positive");
  }
}

void DwellDetectorCore::Reset() {
  active_target_.reset();
  target_start_ms_.reset();
  emitted_for_active_ = false;
}

std::optional<TargetEvent> DwellDetectorCore::Update(std::int64_t timestamp_ms, const std::string& target_id) {
  const std::string token = Trim(target_id);
  if (token.empty()) {
    Reset();
    return std::nullopt;
  ...
  return ParseTargetToEvent(timestamp_ms, token, *target_start_ms_, elapsed);
}

RuntimeM1::RuntimeM1(LinearNormalizer normalizer, KeyboardHitTester hit_tester, DwellDetectorCore dwell_detector)
    : normalizer_(std::move(normalizer)),
      hit_tester_(std::move(hit_tester)),
      dwell_detector_(std::move(dwell_detector)),
      last_gaze_point_(std::nullopt) {}

std::optional<TargetEvent> RuntimeM1::UpdateFrame(const FrameFeatures& frame) {
