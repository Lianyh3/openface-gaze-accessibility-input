...
inline std::vector<double> Solve3x3(const std::vector<std::vector<double>>& a, const std::vector<double>& b) {
...
inline AffineCalibrationFitResult FitAffineCalibration(const std::vector<CalibrationPoint>& points, bool clamp = true) {
  if (points.size() < 3) {
  ...
  const auto x_sol = detail_m2::Solve3x3(normal, rhs_x);
  const auto y_sol = detail_m2::Solve3x3(normal, rhs_y);
...
class EmaSmoother2D {
 public:
  explicit EmaSmoother2D(double alpha = 0.4) : alpha_(alpha), x_(std::nullopt), y_(std::nullopt) {
